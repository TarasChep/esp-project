...
// --- ДОПОМІЖНІ ФУНКЦІЇ ---
void send_to_mqtt(const char *msg)
{
...
void dht_task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        if (b_ok && o_ok)
        {
            ESP_LOGI(TAG, "[%s] Bat: %.1f, Out: %.1f", time_str, t_batt, t_out);

            // 1. Умова "На вулиці < 5" - пріоритетний захист
            if (t_out < 5.0)
            {
                servo_set_angle(0);
                fan_set_state(0);
            }
            // 2. Умова "Bat > Out" та "Bat > 28" (Охолодження)
            else if (t_batt > t_out && t_batt > 28.0)
            {
                servo_set_angle(90);
                fan_set_state(1);
            }
            // 3. Умова "Bat < 22" або "Out >= Bat" (Припинення охолодження)
            else if (t_batt < 22.0 || t_out >= t_batt)
            {
                servo_set_angle(0);
                fan_set_state(0);
            }
